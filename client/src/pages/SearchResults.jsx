...
const SearchResults = () => {
  ...
  const handleClick = (product) => {
    const Path = `/shop/product/${product?._id}`;
    navigate(Path);
  };
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 font-fraunces">
      {/* Breadcrumb */}
      <Breadcrumb items={[{ label: "Home", to: "/" }]} className="mb-6 py-5" />

      {/* Heading */}
      <h1 className="text-2xl md:text-3xl lg:text-4xl text-[#300708] mb-6">
        Showing results for {`“${search}”`}
        <span className="text-gray-500 text-lg md:text-xl font-IBM-Plex">
